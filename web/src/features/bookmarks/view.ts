...
import { sortUrls, treeGroups, visibleGroups, type BookmarkGroup, type BookmarkUrl, type BookmarksUIState } from "./model.js";
...
export interface BookmarkViewActions {
  ...
  showUrlEditor?(record: BookmarkUrl): void;
  refreshMetadataForUrl?(record: BookmarkUrl, button: HTMLButtonElement): void;
  moveUrl?(id: number, sourceGroupId: number, targetGroupId: number, select: HTMLSelectElement): void;
  requestDelete?(record: BookmarkUrl, groupId: number, button: HTMLButtonElement): void;
}

export interface BookmarkViewOptions {
  iconSource(record: BookmarkUrl): string;
  actions: BookmarkViewActions;
}

export function renderGroups(
  root: HTMLElement, count: HTMLElement, groups: readonly BookmarkGroup[],
  ui: BookmarksUIState, options: BookmarkViewOptions,
): void {
...
export function optionLabel(group: BookmarkGroup, depth: number): string {
...
function isDefaultGroup(group: BookmarkGroup): boolean {
  return group.name.toLocaleLowerCase() === "default";
}

export function renderUrlItem(record: BookmarkUrl, currentGroup: BookmarkGroup, groups: readonly BookmarkGroup[], options: BookmarkViewOptions): HTMLLIElement {
  ...
  importantToggle.addEventListener("click", () =>
    options.actions.toggleImportant?.(id, !important, importantToggle)
  );
  ...
  refreshMetadata.addEventListener("click", () =>
    options.actions.refreshMetadataForUrl?.(record, refreshMetadata)
  );
  ...
  move.addEventListener("change", () =>
    options.actions.moveUrl?.(id, currentGroupId, Number(move.value), move)
  );
  ...
  remove.addEventListener("click", () =>
    options.actions.requestDelete?.(record, currentGroupId, remove)
  );
...
export function renderSiteIcon(record: BookmarkUrl, iconSource: (record: BookmarkUrl) => string): HTMLSpanElement {

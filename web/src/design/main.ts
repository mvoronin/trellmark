import { createBookmarksModel, createBookmarkEditors, createBookmarkDrag, renderGroups,
  type BookmarkGroup } from "../features/bookmarks/index.js";
...
const groupsRoot = required<HTMLDivElement>("#groups");
const count = required<HTMLParagraphElement>("#url-count");
const sort = required<HTMLSelectElement>("#sort-select");
let ready = Promise.resolve();

function required<T extends Element>(selector: string): T {
  const element = root.querySelector<T>(selector);
  if (!element) throw new Error(`Missing design element: ${selector}`);
  return element;
}
...
const sections = examples.map(example => {
  ...
  for (const id of example.folded ?? []) model.foldGroup(id);
...
function replace(groups: readonly BookmarkGroup[]): void {
  drag.cancel();
...
function toggleFold(id: number, button: HTMLButtonElement, content: HTMLDivElement): void {
  if (model.ui.foldedGroupIds.has(id)) model.unfoldGroup(id);
  else model.foldGroup(id);
...
function prefixIds(owner: HTMLElement, prefix: string): void {
  const ids = new Map(Array.from(owner.querySelectorAll<HTMLElement>("[id]"), node => [node.id, `${prefix}-${node.id}`]));
  for (const node of owner.querySelectorAll<HTMLElement>("[id]")) node.id = ids.get(node.id)!;
  for (const node of owner.querySelectorAll<HTMLElement>("[for], [aria-controls], [aria-labelledby], [aria-describedby]")) {
    for (const attribute of ["for", "aria-controls", "aria-labelledby", "aria-describedby"]) {
      const value = node.getAttribute(attribute);
      if (value) node.setAttribute(attribute, value.split(" ").map(id => ids.get(id) ?? id).join(" "));
    }
  }
}
...
function render(): void {
  ...
  for (const { example, groups, savedCount } of sections) {
    ...
    renderGroups(groups, savedCount, model.server.groups.filter(group => ids.has(group.id)),
      { ...model.ui, safeMode: example.safeMode ?? false }, options);
...
sort.addEventListener("change", () => { model.setSortMode(sort.value as typeof model.ui.sortMode); render(); });
const filters = root.querySelectorAll<HTMLButtonElement>(".group-filter [data-group-filter]");
...
const launches = [
  ...
  ["confirm-dialog", "URL deletion confirmation", (button: HTMLButtonElement) => {
    const group = editorExample();
    if (!group) return;
    required<HTMLInputElement>("#delete-immediately").checked = false;
    void editors.requestDelete(group.urls[0], group.id, button);
  }],
  ["group-delete-dialog", "Group deletion confirmation", () => {
    const group = editorExample();
    if (group) void editors.requestGroupDelete(group);
  }],
] as const;
function editorExample(): BookmarkGroup | undefined {
  const group = model.server.groups.find(group => group.name !== "default" && group.urls.length > 0);
  if (!group) setStatus("Reload to restore the deleted dialog examples.");
  return group;
}
...
required<HTMLElement>("#app-view").hidden = false;
required<HTMLElement>("#session-pending").hidden = false;
...
const retry = required<HTMLButtonElement>("#import-retry-button");
retry.hidden = false;
retry.disabled = false;
retry.addEventListener("click", () => setStatus("Example import failed. You can retry without losing this view.", "is-error"));
required<HTMLInputElement>("#import-input").addEventListener("change", event => {
...
function keepExampleNavigationLocal(event: MouseEvent): void {

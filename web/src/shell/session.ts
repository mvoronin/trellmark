...
export interface ShellSessionOptions {
  api: Pick<typeof client,
    "login" | "logout" | "getSession" | "clearSession" | "setSessionExpiredHandler">;
...
export function createShellSession(root: ParentNode, options: ShellSessionOptions) {
  ...
  function clear(message = ""): void {
    ...
    queueMicrotask(() => {
      if (!disposed && lifetime.isCurrent(ticket)) view.loginInput.focus();
    });
  }
  ...
  view.form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!lifetime.isCurrent(ticket)) return;
      authenticated();
    } catch (error) {
      if (!lifetime.isCurrent(ticket)) return;
      ...
      if (lifetime.finish(ticket)) view.loginButton.disabled = false;
    }
  ...
  view.logoutButton.addEventListener("click", async () => {
    ...
    try {
      ...
      if (lifetime.isCurrent(ticket)) options.onError(errorMessage(error));
    } finally {
      if (lifetime.finish(ticket)) view.logoutButton.disabled = false;
    }
  }, { signal: controller.signal });
  ...
  return {
    ...
    async start(): Promise<void> {
      ...
      try {
        ...
        if (lifetime.isCurrent(ticket)) clear(errorMessage(error));
      } finally {

...
const pendingView = requiredElement("#session-pending");
const loginView = requiredElement("#login-view");
const appView = requiredElement("#app-view");
const loginForm = requiredElement("#login-form");
const loginInput = requiredElement("#login-input");
const passwordInput = requiredElement("#password-input");
const loginButton = requiredElement("#login-button");
const loginStatus = requiredElement("#login-status");
...
const statusMessage = requiredElement("#form-status");
const count = requiredElement("#url-count");
const deleteImmediately = requiredElement("#delete-immediately");
const confirmDialog = requiredElement("#confirm-dialog");
const confirmUrl = requiredElement("#confirm-url");
...
function treeGroups(source = groups) {
    const result = [];
...
function optionLabel(group, depth) {
...
function syncCreateParentSelect() {
    ...
    fillGroupSelect(groupParent, treeGroups().filter(({ depth }) => depth < 3), pending);
...
function renderGroup(group, depth, visibleUrlIds) {
    const section = document.createElement("section");
    section.className = "group";
    section.dataset.depth = String(depth);
    section.dataset.parentId = group.parent_id === null ? "" : String(group.parent_id);
    section.style.setProperty("--group-hue", String(GROUP_HUES[group.id % GROUP_HUES.length]));
...
async function persistGroupOrder(parentId, orderedIds) {
    ...
    try {
        ...
        loadGroups().catch((loadError) => setStatus(errorMessage(loadError), "is-error"));
    }
}
function renderUrlItem(record, currentGroup) {
    ...
    importantToggle.setAttribute("aria-label", `Mark ${displayedUrl} important in ${currentGroup.name}`);
    importantToggle.append(icon("star"));
    importantToggle.addEventListener("click", () => toggleImportant(id, !important, importantToggle));
    const edit = document.createElement("button");
    edit.type = "button";
    edit.className = "url-action edit-url-button";
    edit.setAttribute("aria-label", `Edit ${displayedUrl} in ${currentGroup.name}`);
    edit.append(icon("pencil"));
    ...
    refreshMetadata.setAttribute("aria-label", `Refresh title and icon for ${displayedUrl} in ${currentGroup.name}`);
    refreshMetadata.append(icon("refresh-cw"));
    refreshMetadata.addEventListener("click", () => refreshMetadataForUrl(record, refreshMetadata));
    const move = document.createElement("select");
    move.className = "move-select";
    move.setAttribute("aria-label", `Move ${displayedUrl} from ${currentGroup.name}`);
    ...
    move.addEventListener("change", () => moveUrl(id, currentGroupId, Number(move.value), move));
    const remove = document.createElement("button");
    remove.type = "button";
    remove.className = "delete-button";
    remove.append(icon("trash-2"));
    remove.setAttribute("aria-label", `Delete ${displayedUrl} from ${currentGroup.name}`);
    remove.addEventListener("click", () => requestDelete(record, currentGroupId, remove));
...
for (const button of groupFilterButtons) {
    button.addEventListener("click", () => {
        ...
        for (const candidate of groupFilterButtons) {
            candidate.setAttribute("aria-pressed", String(candidate.dataset.groupFilter === groupFilter));
        }
        renderGroups(groups);
    });
}
...
function showAuthenticated(csrfToken) {
...
function editParentCandidates(group) {
    ...
    return treeGroups().filter(({ group: candidate, depth }) => !excluded.has(candidate.id)
...
function isCurrentUrlEditRequest(requestSerial, urlId) {
    return (requestSerial === urlEditRequestSerial &&
        editingUrl?.id === urlId &&
        urlEditDialog.open);
}
...
function confirmGroupDeletion(group) {
    ...
    return new Promise((resolve) => {
        groupDeleteDialog.addEventListener("close", () => {
            ...
            resolve(action === "delete" || action === "move_to_default" ? action : null);
        }, { once: true });
    });
}

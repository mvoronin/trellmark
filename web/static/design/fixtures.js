...
export function localIconSource(id) {
    if (id === 120)
        return "/static/design/no-icon.svg";
    if (id === 121)
        return "/static/design/frame.css";
...
export function createFixtureApi(seed = initialGroups()) {
    ...
    let nextId = 1000;
    const walk = (nodes = groups) => nodes.flatMap(group => [group, ...walk(group.children)]);
    const payload = () => ({ groups: structuredClone(groups) });
    const findGroup = (id) => {
        const group = walk().find(group => group.id === id);
        if (!group)
            throw new Error("This example group no longer exists. Reload to reset.");
        return group;
    };
    const findUrl = (id) => {
        const url = walk().flatMap(group => group.urls).find(url => url.id === id);
        if (!url)
            throw new Error("This example URL no longer exists. Reload to reset.");
        return url;
    };
    const siblings = (parentId) => parentId === null ? groups : findGroup(parentId).children;
    function safeUrl(value) {
        const url = new URL(value);
        if (url.protocol !== "https:" && url.protocol !== "http:")
            throw new Error("Use an HTTP or HTTPS example URL.");
        return url.href;
    }
    const api = {
        async listGroups() { return payload(); },
        siteIconPath: localIconSource,
        async setUrlImportant(id, body) {
            const url = findUrl(id);
            url.important = body.important;
            return { ...payload(), url: structuredClone(url) };
        },
        async editUrl(id, body) {
            const url = findUrl(id);
            if (body.version !== url.version)
                throw new Error("This URL was changed. Reload and try again.");
        ...
        async moveUrlToGroup(id, body) {
            ...
            if (!target.urls.some(url => url.id === id))
        ...
        async createGroup(body) {
            ...
            if (parent?.depth === 3)
                throw new Error("Examples support three group depths.");
        ...
        async editGroup(id, body) {
            const group = findGroup(id);
            if (group.name === "default")
                throw new Error("The default group is protected.");
        ...
        async deleteGroup(id, body) {
            ...
            const source = siblings(group.parent_id);
            source.splice(source.indexOf(group), 1);
            if (body.url_action === "move_to_default")
                findGroup(2).urls.push(...urls);
